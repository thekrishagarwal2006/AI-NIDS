...
def initialize_explainer(model, model_name, X_background):
    ...
    if "Forest" in model_name or "XGBoost" in model_name or hasattr(model, "feature_importances_"):
        ...
        # Use representative background sample (50 instances) for KernelExplainer
...
def compute_shap_explanations(model, model_name, X_test, feature_names, label_encoder, X_train=None, results_dir="results/shap"):
    ...
    if isinstance(explainer, shap.TreeExplainer):
        ...
        shap_values = explainer.shap_values(df_sample, nsamples=50)
...
def explain_single_prediction(model, sample_vector, feature_names, label_encoder, explainer=None, X_background=None, sample_idx=0, results_dir="results/shap"):
    """
    ...
    if explainer is None:
        bg = X_background if X_background is not None else np.tile(sample_vector, (20, 1))
        explainer = initialize_explainer(model, "Best Model", bg)
    ...
    if isinstance(explainer, shap.TreeExplainer):
        ...
        sv_raw = explainer.shap_values(sample_df, nsamples=50)
        if isinstance(sv_raw, list):
            sv = sv_raw[pred_class_idx][0]
        elif isinstance(sv_raw, np.ndarray) and sv_raw.ndim == 3:
            sv = sv_raw[0, :, pred_class_idx]
    ...
    explanation_text.append("========================================")
    explanation_text.append(f"Predicted Class: {pred_class_name.upper()}")
    ...
    for idx, (feat_name, val, shap_val) in enumerate(feature_contributions[:5], 1):
        ...
        explanation_text.append(
            f"{idx}. {feat_name} (scaled val: {val:.3f}) : {sign}{shap_val:.4f} ({direction} probability of {pred_class_name})"
        )
    ...
    if top_pos:
        explanation_text.append(
            f"The prediction of {pred_class_name.upper()} was primarily driven by feature(s): {', '.join(top_pos)}."
        )
    else:
        explanation_text.append(f"The classification of {pred_class_name.upper()} was derived from feature interactions across the network flow.")
